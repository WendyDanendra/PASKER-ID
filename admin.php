...
$view = $_GET['view'] ?? 'directory_individual'; // directory_individual | verifikasi_employer | verifikasi_job
$entity = $_GET['entity'] ?? 'Individu'; // Semua | Perusahaan | Individu
$tab = $_GET['tab'] ?? 'all';
$search = trim($_GET['q'] ?? '');
...
$verificationEmployers = $verificationEmployers ?? [];
$verificationJobs = $verificationJobs ?? [];

$adminName = $user['name'] ?? 'Admin Pusat';
$adminInitials = '';
...
$currentTitle = $viewTitles[$view] ?? 'Direktori Profil';
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<div class="app-shell">
    <aside class="sidebar">
        <div class="brand">
            <div class="brand-mark"><span style="font-weight:800;font-size:22px;line-height:1;">T</span></div>
        ...
        <div class="sidebar-footer">
            <div class="sidebar-theme">
                <span class="sidebar-theme-label"><i class="fa-solid fa-circle-half-stroke"></i> Tema</span>
                <div class="sidebar-theme-options">
                    <button type="button" data-theme-option="light" class="ghost-btn">Terang</button>
                    <button type="button" data-theme-option="dark" class="ghost-btn">Gelap</button>
                    <button type="button" data-theme-option="system" class="ghost-btn">Sistem</button>
                </div>
            </div>
            <button type="button" class="sidebar-theme-compact" data-theme-cycle title="Ubah tema tampilan">
                <i class="fa-solid fa-display"></i>
            </button>
            <button type="button" class="profile-card" data-open-modal="modal-user-account" title="Pengaturan akun pengguna">
                <div class="profile-avatar"><?php echo e($adminInitials); ?></div>
                <div>
                    <strong><?php echo e($adminName); ?></strong>
                    <span>Admin pusat</span>
                </div>
                <i class="fa-solid fa-chevron-right" style="color:#cbd5e1; font-size:11px; margin-left:auto;"></i>
            </button>
        </div>
    </aside>
    ...
    <div class="main">
        <header class="topbar">
            <button id="sidebarToggle" class="sidebar-toggle" type="button" aria-label="Ciutkan menu"><i class="fa-solid fa-bars"></i></button>
            <div class="crumbs">
                <span>Beranda</span>
                <span>&gt;</span>
                <strong><?php echo e($currentTitle); ?></strong>
            </div>
...
<div class="modal-backdrop" data-modal="modal-user-account">
    <div class="popup-dialog-card" style="text-align:left;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px;">
            <h3 style="font-size:16px; font-weight:800;">Akun Pengguna</h3>
            <button type="button" class="ghost-btn" data-close-modal="modal-user-account">×</button>
        </div>
        <div style="font-size:13px; display:grid; gap:10px; margin-bottom:20px;">
            <div><span style="color:#64748b;">Nama:</span> <strong><?php echo e($adminName); ?></strong></div>
            <div><span style="color:#64748b;">Email:</span> <strong><?php echo e($user['email'] ?? ''); ?></strong></div>
            <div><span style="color:#64748b;">Peran:</span> <strong>Admin Pusat</strong></div>
        </div>
        <a href="logout.php" class="ghost-btn" style="width:100%; justify-content:center; color:#dc2626; border-color:#fecaca;">
...
<script>
    (function () {
        ...
        const openOverlay = (selector) => document.querySelector(selector)?.classList.add('open');
        const closeOverlay = (el) => el?.classList.remove('open');

        document.querySelectorAll('[data-open-modal]').forEach((btn) => {
            btn.addEventListener('click', (event) => {
                event.preventDefault();
                openOverlay(`[data-modal="${btn.dataset.openModal}"]`);
            });
        });
        document.querySelectorAll('[data-close-modal]').forEach((btn) => {
            btn.addEventListener('click', (event) => {
                event.preventDefault();
                closeOverlay(document.querySelector(`[data-modal="${btn.dataset.closeModal}"]`));
            });
        });
        document.querySelectorAll('[data-open-drawer]').forEach((btn) => {
            btn.addEventListener('click', (event) => {
                event.preventDefault();
                openOverlay(`[data-drawer="${btn.dataset.openDrawer}"]`);
            });
        });
        document.querySelectorAll('[data-close-drawer]').forEach((btn) => {
            btn.addEventListener('click', (event) => {
                event.preventDefault();
                closeOverlay(document.querySelector(`[data-drawer="${btn.dataset.closeDrawer}"]`));
            });
        });
